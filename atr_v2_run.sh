#!/bin/bash
# 第二版纸上运行的看护：进程异常退出即重启，时限到达即停。
#   ./atr_v2_run.sh maker all 172800
# journal 追加写入，重启不丢已有记录；心跳里的计数器每次重启归零，校验脚本按此处理。
# sandbox 模拟撮合，不下真实订单。
set -u
DIR="$(cd "$(dirname "$0")" && pwd)"
ARM="${1:?用法: atr_v2_run.sh maker|taker thick|all [秒]}"
RULES="${2:?用法: atr_v2_run.sh maker|taker thick|all [秒]}"
DURATION="${3:-172800}"
BIN="$DIR/target-v2/debug/examples/polymarket-atr-margin-sandbox"
LOGDIR="$DIR/v2_logs"; SUP="$LOGDIR/supervisor_${ARM}_${RULES}.log"
